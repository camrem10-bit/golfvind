...
- Automatic reload after a new service worker takes over, plus a visible version number (5.31) and Check for updates button inside the collapsible How it works card. Slope Compensation and Wind have separate collapsible cards; the club editing section is labelled Edit: My bag. The bilingual How it works text presents four setup and usage steps in larger, higher-contrast type, including how to add the app to an iPhone Home Screen. A collapsible feedback form opens an email draft to info@gdop.se for suggestions or problems; the visitor sends it from their own mail app.
